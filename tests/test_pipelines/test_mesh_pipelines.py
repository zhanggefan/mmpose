...
def test_mesh_pipeline():
    ...
    assert isinstance(results_tensor['index_mask'], torch.LongTensor)
    assert results_tensor['index_mask'].shape == torch.Size([1, 64, 64])
    max_I = results_tensor['index_mask'].max().item()
    ...
    assert isinstance(results_no_iuv['index_mask'], torch.LongTensor)
    assert results_no_iuv['index_mask'].shape == torch.Size([1, 64, 64])
    max_I = results_no_iuv['index_mask'].max().item()
    assert (max_I == 0)
    ...
    collect = Collect(
        keys=[
            ...
            'joints_3d_visible', 'pose', 'beta', 'index_mask', 'uv_coordinates'
        ],
