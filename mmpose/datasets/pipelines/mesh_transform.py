...
class IUVToTensor():
    ...
    Modifies key: 'index_mask', 'uv_coordinates'.
    ...
    def __call__(self, results):
        ...
        if iuv is None:
            ...
            index_mask = torch.zeros([1, H, W]).long()
            uv_coordinates = torch.zeros([2, H, W]).float()
        else:
            index_mask = torch.LongTensor(iuv[:, :, 0])[None, :, :]
            uv_coordinates = torch.FloatTensor(iuv[:, :, 1:]) / 255
            uv_coordinates = uv_coordinates.permute(2, 0, 1)
        results['index_mask'] = index_mask
